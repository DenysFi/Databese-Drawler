...
const TopHeaderMenuItem: FC<ITopHeaderMenuItem> = ({ item, itemName }) => {
    ...
    const activeHoverColor = mode === 'light' ? 'hover:bg-[#cbd5e1]' : 'hover:bg-[#5e5f5f]'
    return (
        <Dropdown
            position='bottomLeft'
                                ...
                                return (
                                    <Dropdown.Item key={key} onClick={value.function} >
                                        <div className='flex justify-between w-full items-center'>
                                            <div >{key}</div>
                                            <div className=' text-xs text-gray-400'>{value.shortcast}</div>
                                        </div>
                                    </Dropdown.Item>
                                )
                            }
        ...
        >
            <div className={`px-[12px] py-[4px] ${activeHoverColor} rounded cursor-pointer ${activeTextColor}`}>{itemName}</div>
        </Dropdown>
    )
};
